...
```
DATABASE_URL="postgresql://user:password@localhost:5432/stormeye?schema=public"
```

**Exemple avec PostgreSQL local :**

```
DATABASE_URL="postgresql://postgres:password@localhost:5432/stormeye?schema=public"
```

**Exemple avec Docker :**

```bash
docker run --name stormeye-db -e POSTGRES_PASSWORD=password -p 5432:5432 -d postgres:latest
# Puis créer la base de données:
# docker exec stormeye-db createdb -U postgres stormeye
```
...
**Erreur : "database "stormeye" does not exist"**

- Créez la base de données : `createdb stormeye`
...
- Vérifiez les imports : `import { PrismaClient } from '@prisma/client'`
