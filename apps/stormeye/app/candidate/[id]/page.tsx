      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="grid gap-8 xl:grid-cols-[0.9fr_1.1fr]">
          <section className="rounded-[2rem] bg-slate-900/90 p-8 ring-1 ring-white/10">
            ...
            <div className="mt-5 space-y-4 text-sm text-slate-300">
              <p>Email : {candidate.email ?? "Non renseigne"}</p>
              <p>WhatsApp : {candidate.phoneNumber ?? "Non renseigne"}</p>
              <p>CV : {candidate.resumeUrl ?? "Non renseigne"}</p>
              {interviewUrl ? (
        ...
        <section className="rounded-[2rem] bg-slate-900/90 p-8 ring-1 ring-white/10">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <GenerateReportButton candidateId={candidate.id} />
          </div>
        ...
        <section className="rounded-[2rem] bg-slate-900/90 p-8 ring-1 ring-white/10">
          ...
          <div className="mt-6 space-y-4">
            {candidate.conversations[0]?.messages.map((message) => (
              <div
                key={message.id}
